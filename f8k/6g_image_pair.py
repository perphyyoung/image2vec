...
def load_image(group_name, pair_file, image_size):
    ...
    root_path_image = '/data/home/py/data/flickr8k/images/'
    lst_image = list()
    lst_caption = list()
    i = 0
...
def gen_pair(path_npz, path_hdf5, image_size):
    if not os.path.exists(path_npz):
        os.mkdir(path_npz)

    lst_group_names = ['train', 'test']
    fpath = os.path.join(path_hdf5, 'filtered_8.hdf5')
...
if __name__ == '__main__':
    ...
    path_hdf5 = './hdf5/'
    image_size = 224
    gen_pair(path_npz, path_hdf5, image_size)
